...
pub async fn create_task_tag(
    ...
    task_facades::add_task_tag_relation(&*repositories, &project_id, &task_id, &tag_id).await
}
...
pub async fn delete_task_tag(
    ...
    task_facades::remove_task_tag_relation(&*repositories, &project_id, &task_id_typed, &tag_id_typed).await
}
...
pub async fn create_task_tag_by_name(
    ...
    let repositories = state.repositories.read().await;
    task_facades::add_task_tag(&*repositories, &project_id, &task_id, &tag_name).await
}
...
pub async fn create_subtask_tag_by_name(
    ...
    let repositories = state.repositories.read().await;
    subtask_facades::add_subtask_tag_by_name(&*repositories, &project_id, &subtask_id, &tag_name).await
}
...
pub async fn create_subtask_tag(
    ...
    subtask_facades::add_subtask_tag_relation(&*repositories, &project_id, &subtask_id, &tag_id).await
}
