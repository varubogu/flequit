...
impl DateConditionLocalAutomergeRepository {
    ...
    #[tracing::instrument(level = "trace")]
    pub async fn list_date_conditions(&self, project_id: &ProjectId) -> Result<Vec<DateCondition>, RepositoryError> {
        let document = self.get_or_create_document(project_id).await?;
        let date_conditions = document.load_data::<Vec<DateCondition>>("date_conditions").await?;
    ...
    /// IDで日付条件を取得
    #[tracing::instrument(level = "trace")]
    pub async fn get_date_condition(&self, project_id: &ProjectId, condition_id: &str) -> Result<Option<DateCondition>, RepositoryError> {
        let date_conditions = self.list_date_conditions(project_id).await?;
        Ok(date_conditions.into_iter().find(|d| d.id == condition_id.into()))
    }

    /// 日付条件を作成または更新
    #[tracing::instrument(level = "trace")]
    pub async fn set_date_condition(&self, project_id: &ProjectId, date_condition: &DateCondition) -> Result<(), RepositoryError> {
        log::info!("set_date_condition - 開始: {:?}", date_condition.id);
        let mut date_conditions = self.list_date_conditions(project_id).await?;
        log::info!("set_date_condition - 現在の日付条件数: {}", date_conditions.len());

        // 既存の日付条件を更新、または新規追加
        if let Some(existing) = date_conditions.iter_mut().find(|d| d.id == date_condition.id) {
            log::info!("set_date_condition - 既存日付条件を更新: {:?}", date_condition.id);
            *existing = date_condition.clone();
        } else {
            log::info!("set_date_condition - 新規日付条件追加: {:?}", date_condition.id);
            date_conditions.push(date_condition.clone());
        }

        let document = self.get_or_create_document(project_id).await?;
        log::info!("set_date_condition - Document取得完了");
        let result = document.save_data("date_conditions", &date_conditions).await;
        match result {
            Ok(_) => {
            ...
            Err(e) => {
                log::error!("set_date_condition - Automergeドキュメント保存エラー: {:?}", e);
    ...
    /// 日付条件を削除
    #[tracing::instrument(level = "trace")]
    pub async fn delete_date_condition(&self, project_id: &ProjectId, condition_id: &str) -> Result<bool, RepositoryError> {
        let mut date_conditions = self.list_date_conditions(project_id).await?;
        ...
        if date_conditions.len() != initial_len {
            let document = self.get_or_create_document(project_id).await?;
            document.save_data("date_conditions", &date_conditions).await?;
            Ok(true)
        } else {
            Ok(false)
        }
...
impl ProjectRepository<DateCondition, DateConditionId> for DateConditionLocalAutomergeRepository {
    ...
    async fn save(&self, project_id: &ProjectId, entity: &DateCondition) -> Result<(), RepositoryError> {
        log::info!("DateConditionLocalAutomergeRepository::save - 開始: {:?}", entity.id);
        let result = self.set_date_condition(project_id, entity).await;
        if result.is_ok() {
            log::info!("DateConditionLocalAutomergeRepository::save - 完了: {:?}", entity.id);
        } else {
            log::error!("DateConditionLocalAutomergeRepository::save - エラー: {:?}", result);
        }
        result
    }
    ...
    async fn delete(&self, project_id: &ProjectId, id: &DateConditionId) -> Result<(), RepositoryError> {
        let deleted = self.delete_date_condition(project_id, &id.to_string()).await?;
        if deleted {
            Ok(())
        } else {
            Err(RepositoryError::NotFound(format!("DateCondition not found: {}", id)))
        }
    }
