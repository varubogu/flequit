...
impl MemberLocalAutomergeRepository {
    ...
    #[tracing::instrument(level = "trace")]
    pub async fn list_members(&self, project_id: &ProjectId) -> Result<Vec<Member>, RepositoryError> {
    ...
    /// IDでメンバーを取得
    #[tracing::instrument(level = "trace")]
    pub async fn get_member(&self, project_id: &ProjectId, user_id: &str) -> Result<Option<Member>, RepositoryError> {
        let members = self.list_members(project_id).await?;
        Ok(members.into_iter().find(|m| m.user_id.to_string() == user_id))
    }

    /// メンバーを作成または更新
    #[tracing::instrument(level = "trace")]
    pub async fn set_member(&self, project_id: &ProjectId, member: &Member) -> Result<(), RepositoryError> {
    ...
    /// メンバーを削除
    #[tracing::instrument(level = "trace")]
    pub async fn delete_member(&self, project_id: &ProjectId, user_id: &str) -> Result<bool, RepositoryError> {
        let mut members = self.list_members(project_id).await?;
...
impl ProjectRepository<Member, UserId> for MemberLocalAutomergeRepository {
    ...
    async fn save(&self, project_id: &ProjectId, entity: &Member) -> Result<(), RepositoryError> {
        log::info!("MemberLocalAutomergeRepository::save - 開始: {:?}", entity.id);
        let result = self.set_member(project_id, entity).await;
        if result.is_ok() {
            log::info!("MemberLocalAutomergeRepository::save - 完了: {:?}", entity.id);
        } else {
            log::error!("MemberLocalAutomergeRepository::save - エラー: {:?}", result);
        }
        result
    }
    ...
    async fn delete(&self, project_id: &ProjectId, id: &UserId) -> Result<(), RepositoryError> {
        ...
        if deleted {
            ...
            Err(RepositoryError::NotFound(format!("Member not found: {}", id)))
        }
    }
