...
fn create_test_output_directories() {
    ...
    println!("cargo:warning=📁 テスト出力ディレクトリ作成完了: {}", output_dir.display());
    ...
    if let Err(e) = std::fs::create_dir_all(&automerge_dir) {
        ...
        println!("cargo:warning=📁 automergeディレクトリ作成完了: {}", automerge_dir.display());
    }
    
    if let Err(e) = std::fs::create_dir_all(&json_dir) {
        println!("cargo:warning=❌ jsonサブディレクトリ作成失敗: {}", e);
    } else {
        println!("cargo:warning=📁 jsonディレクトリ作成完了: {}", json_dir.display());
    }
