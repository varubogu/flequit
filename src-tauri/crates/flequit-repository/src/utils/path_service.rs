...
impl PathService {
    ...
    /// 設定をファイルから読み込み
    pub async fn load_config() -> Result<PathConfig, Box<dyn std::error::Error>> {
        let config_file = Self::get_config_file()?;
    ...
    /// システムデフォルトに戻す
    pub async fn reset_to_system_default(&mut self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// パスの正当性をチェック
    pub async fn validate_path(&self, path: &PathBuf) -> Result<bool, Box<dyn std::error::Error>> {
        // パスが存在するか、作成可能かチェック
        if path.exists() {
            // 書き込み権限があるかチェック
