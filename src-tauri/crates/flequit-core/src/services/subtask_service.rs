...
pub async fn create_subtask<R>(repositories: &R, project_id: &ProjectId, subtask: &SubTask) -> Result<(), ServiceError>
where
...
#[tracing::instrument(level = "trace")]
pub async fn get_subtask<R>(repositories: &R, project_id: &ProjectId, subtask_id: &SubTaskId) -> Result<Option<SubTask>, ServiceError>
where
    R: InfrastructureRepositoriesTrait + Send + Sync, {
...
#[tracing::instrument(level = "trace")]
pub async fn list_subtasks<R>(repositories: &R, project_id: &ProjectId, task_id: &str) -> Result<Vec<SubTask>, ServiceError>
where
    R: InfrastructureRepositoriesTrait + Send + Sync, {
...
#[tracing::instrument(level = "trace")]
pub async fn delete_subtask<R>(repositories: &R, project_id: &ProjectId, subtask_id: &SubTaskId) -> Result<(), ServiceError>
where
    R: InfrastructureRepositoriesTrait + Send + Sync, {
...
#[tracing::instrument(level = "trace")]
pub async fn toggle_completion<R>(repositories: &R, project_id: &ProjectId, subtask_id: &SubTaskId) -> Result<(), ServiceError>
where
    R: InfrastructureRepositoriesTrait + Send + Sync, {
    // 既存のサブタスクを取得
