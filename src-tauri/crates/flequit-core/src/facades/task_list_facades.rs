    ...
    R: InfrastructureRepositoriesTrait + TransactionManager<Transaction = DatabaseTransaction> + Send + Sync,
{
    ...
    // 3. タスクリスト本体を削除
    if let Err(e) = sqlite_repos_guard.task_lists.delete_with_txn(&txn, project_id, id).await {
    ...
    if let Some(automerge) = repositories.automerge_repositories() {
        ...
        if let Err(e) = automerge_guard.projects.mark_task_list_deleted(project_id, id, user_id, timestamp).await {
    ...
    R: InfrastructureRepositoriesTrait + Send + Sync,
{
    ...
    let deleted_task_list = match automerge_guard.projects.get_deleted_task_list_by_id(project_id, id).await {
        Ok(Some(tl)) => tl,
    ...
    if let Err(e) = automerge_guard.projects.restore_task_list(project_id, id, user_id, timestamp).await {
