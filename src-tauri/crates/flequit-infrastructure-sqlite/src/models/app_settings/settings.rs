...
impl SqliteModelConverter<Settings> for Model {
    async fn to_domain_model(&self) -> Result<Settings, String> {
        ...
        let custom_date_formats: Vec<DateTimeFormat> =
        ...
        Ok(Settings {
            ...
            date_format: serde_json::from_str(&self.custom_date_formats)
                .unwrap_or_default(),
            custom_due_days,
            time_labels,
...
impl DomainToSqliteConverter<ActiveModel> for Settings {
    async fn to_sqlite_model(&self) -> Result<ActiveModel, String> {
        ...
        let custom_date_formats_json = serde_json::to_string(&self.date_format)
