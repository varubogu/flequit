...
impl SubTaskLocalSqliteRepository {
    ...
    pub async fn find_by_task(&self, task_id: &str) -> Result<Vec<SubTask>, RepositoryError> {
        ...
        let models = SubtaskEntity::find()
            ...
            .all(db)
            .await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
...
impl ProjectRepository<SubTask, SubTaskId> for SubTaskLocalSqliteRepository {
    async fn save(&self, project_id: &ProjectId, subtask: &SubTask) -> Result<(), RepositoryError> {
        ...
        // トランザクション開始
        let txn = db.begin().await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
        ...
        if existing.is_some() {
            // 既存レコードがある場合は更新
            active_model.update(&txn).await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
        } else {
            // 既存レコードがない場合は挿入
            active_model.insert(&txn).await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
        }
        ...
        // 有効なタグIDのみで紐づけを更新
        self.subtask_tag_repository.update_subtask_tag_relations(&txn, &subtask.id, &valid_tag_ids).await.map_err(|e| RepositoryError::from(e))?;

        txn.commit().await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
        Ok(())
    }

    async fn find_by_id(&self, _project_id: &ProjectId, id: &SubTaskId) -> Result<Option<SubTask>, RepositoryError> {
        let db_manager = self.db_manager.read().await;
        let db = db_manager.get_connection().await.map_err(|e| RepositoryError::from(e))?;

        if let Some(model) = SubtaskEntity::find_by_id(id.to_string()).one(db).await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))? {
            let mut subtask = model
                .to_domain_model()
                .await
                .map_err(|e: String| RepositoryError::from(SQLiteError::ConversionError(e)))?;
    ...
    async fn delete(&self, _project_id: &ProjectId, id: &SubTaskId) -> Result<(), RepositoryError> {
        ...
        // サブタスク本体を削除
        SubtaskEntity::delete_by_id(id.to_string()).exec(&txn).await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;

        txn.commit().await.map_err(|e| RepositoryError::from(SQLiteError::from(e)))?;
        Ok(())
    }

    async fn exists(&self, _project_id: &ProjectId, id: &SubTaskId) -> Result<bool, RepositoryError> {
        let db_manager = self.db_manager.read().await;
        let db = db_manager.get_connection().await.map_err(|e| RepositoryError::from(e))?;
