...
export class RecurrenceRuleWebService implements RecurrenceRuleService {
  ...
  async update(projectId: string, ruleId: string, patch: RecurrenceRulePatch, userId: string): Promise<boolean> {
    // TODO: Web API実装を追加
    console.warn('Web backends: updateRecurrenceRule not implemented', { projectId, ruleId, patch, userId });
    return true; // 警告を出しつつ正常終了として扱う
  }

  async delete(projectId: string, ruleId: string, userId: string): Promise<boolean> {
    // TODO: Web API実装を追加
    console.warn('Web backends: deleteRecurrenceRule not implemented', { projectId, ruleId, userId });
    return true; // 警告を出しつつ正常終了として扱う
  }
