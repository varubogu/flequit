import type { Account } from '$lib/types/account';
import type { AccountService } from '$lib/infrastructure/backends/account-service';

export class AccountWebService implements AccountService {
  async create(account: Account): Promise<boolean> {
    // TODO: Web API実装を追加
    console.warn('Web backends: createAccount not implemented', account);
    return false; // 仮実装として失敗を返す
  }

  async get(id: string): Promise<Account | null> {
    // TODO: Web API実装を追加
    console.warn('Web backends: getAccount not implemented', id);
    return null; // 仮実装としてnullを返す
  }

  async update(id: string, patch: Partial<Account>): Promise<boolean> {
    // TODO: Web API実装を追加
    console.warn('Web backends: updateAccount not implemented', id, patch);
    return false; // 仮実装として失敗を返す
  }

  async delete(id: string): Promise<boolean> {
    // TODO: Web API実装を追加
    console.warn('Web backends: deleteAccount not implemented', id);
    return false; // 仮実装として失敗を返す
  }
}
