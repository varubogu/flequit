<script lang="ts">
  ...
  function getRecurrenceText(rule: RecurrenceRule | null | undefined): string {
    ...
    let text = translationService.getMessage('every_interval_unit', { interval: String(interval), unit: unitText })();
    ...
    if (rule.unit === 'month') {
      if (rule.details?.specific_date) {
        ...
        const detail = translationService.getMessage('day_of_month', { day: dayOfMonth })() + getOrdinalSuffix(dayOfMonth);
    ...
    if (rule.end_date) {
      ...
      text += ' ' + translationService.getMessage('recurrence_max_occurrences', { count: rule.max_occurrences })();
    }
