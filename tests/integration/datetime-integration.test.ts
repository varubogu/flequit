...
import type { TaskWithSubTasks, RecurrenceRule } from '$lib/types/task';
...
const mockTaskDateStore = {
  ...
  setTaskDate: vi.fn((taskId: string, dateData: {
    start_date?: Date;
    end_date?: Date;
...
describe('日付・時刻管理結合テスト', () => {
  ...
  it('繰り返しタスクの日付生成が正しく動作する', () => {
    ...
    expect(mockRecurrenceService.generateRecurrenceDates).toHaveBeenCalledWith(baseDate, dailyRule, 5);
    ...
    const complexDateFilter = (conditions: {
      ...
      if (conditions.dateRange) {
        results = results.filter(task => {
          if (!task.end_date) return false;
          return task.end_date >= conditions.dateRange!.start && 
