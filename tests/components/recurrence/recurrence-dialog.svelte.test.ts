...
// vitest.setup.tsの統一的なモック化を使用するため、個別モック化は削除
...
describe('RecurrenceDialog', () => {
  ...
  it.skip('保存時に正しいルールが生成される', async () => {
    render(RecurrenceDialog, {
      props: {
        open: true,
        onSave: mockOnSave,
  ...
  it.skip('無効選択時はnullが保存される', async () => {
    render(RecurrenceDialog, {
      props: {
        open: true,
        onSave: mockOnSave,
  ...
  it.skip('繰り返し回数設定時にmax_occurrencesが保存される', async () => {
    render(RecurrenceDialog, {
      props: {
        open: true,
        onSave: mockOnSave,
  ...
  describe('Recurrence Count Input Validation', () => {
    ...
    it.skip('小数点を入力すると整数に丸められる', async () => {
