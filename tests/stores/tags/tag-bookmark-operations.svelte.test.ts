...
describe('TagBookmarkOperations', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(TagBookmarkService.create).mockImplementation(
      async (_projectId: string, tagId: string) => {
    ...
    vi.mocked(TagBookmarkService.delete).mockImplementation(
      async (_bookmarkId: string, tagId: string) => {
        const filtered = bookmarkStore.bookmarks.filter((b) => b.tagId !== tagId);
        bookmarkStore.setBookmarks(filtered);
      }
    );

    vi.mocked(TagBookmarkService.toggleBookmark).mockImplementation(
      async (_projectId: string, tagId: string) => {
